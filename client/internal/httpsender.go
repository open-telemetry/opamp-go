...
func (h *HTTPSender) makeOneRequestRoundtrip(ctx context.Context) {
	...
	if err != nil {
		h.logger.ErrorfContext(ctx, "%v", err)
		return
	}
	if resp == nil {
...
func (h *HTTPSender) sendRequestWithRetries(ctx context.Context) (*http.Response, error) {
	...
	if err != nil {
		if errors.Is(err, context.Canceled) {
			h.logger.DebugfContext(ctx, "Client is stopped, will not try anymore.")
		} else {
			h.logger.ErrorfContext(ctx, "Failed prepare request (%v), will not try anymore.", err)
		}
		return nil, err
	}
	if req == nil {
		...
		case <-timer.C:
			{
				...
				h.logger.ErrorfContext(ctx, "Failed to do HTTP request (%v), will retry", err)
...
func (h *HTTPSender) prepareRequest(ctx context.Context) (*requestWrapper, error) {
	...
	if h.compressionEnabled {
		...
		if _, err = g.Write(data); err != nil {
			h.logger.ErrorfContext(ctx, "Failed to compress message: %v", err)
			return nil, err
		}
		if err = g.Close(); err != nil {
			h.logger.ErrorfContext(ctx, "Failed to close the writer: %v", err)
			return nil, err
		}
...
func (h *HTTPSender) receiveResponse(ctx context.Context, resp *http.Response) {
	...
	if err != nil {
		_ = resp.Body.Close()
		h.logger.ErrorfContext(ctx, "cannot read response body: %v", err)
		return
	}
	_ = resp.Body.Close()
	...
	if err := proto.Unmarshal(msgBytes, &response); err != nil {
		h.logger.ErrorfContext(ctx, "cannot unmarshal response: %v", err)
		return
	}
